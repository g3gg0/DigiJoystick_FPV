...
#define HID_REPORT_SIZE (6+2)

/* What was most recently read from the controller */
unsigned char last_built_report[HID_REPORT_SIZE];
/* What was most recently sent to the host */
unsigned char last_sent_report[HID_REPORT_SIZE];

// report frequency set to default of 50hz
#define DIGIJOYSTICK_DEFAULT_REPORT_INTERVAL 80
...
const unsigned char gcn64_usbHidReportDescriptor[USB_CFG_HID_REPORT_DESCRIPTOR_LENGTH] PROGMEM = {
...
class DigiJoystick_FPV {
 public:
	...
	/* returns true if there is data to be sent */
	bool update() {
		usbPoll();
		
		// instead of above code, use millis arduino system to enforce minimum reporting frequency
		unsigned long current_millis = millis();
		unsigned long time_since_last_report = current_millis - last_report_time;

		if (time_since_last_report >= 100) {
			must_report = 1;
		}
		
		// if the report has changed, try force an update anyway
		if (memcmp(last_built_report, last_sent_report, HID_REPORT_SIZE)) {
			must_report = 1;
		}
		...
		if (must_report) {
			if (usbInterruptIsReady()) {
				must_report = 0;
				last_report_time = current_millis;
				memcpy(last_sent_report, last_built_report, HID_REPORT_SIZE);
				usbSetInterrupt(last_sent_report, HID_REPORT_SIZE);
			}
			return true;
		}
		return false;
	...
	uchar usbFunctionSetup(uchar data[8]) {
		...
		if ((rq->bmRequestType & USBRQ_TYPE_MASK) == USBRQ_TYPE_CLASS) { // class request type
			if (rq->bRequest == USBRQ_HID_GET_REPORT){ // wValue: ReportType (highbyte), ReportID (lowbyte)
				...
				usbMsgPtr = (usbMsgPtr_t)last_sent_report;
